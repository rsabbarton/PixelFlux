...
import { PixelBrush, FUNC_PAINT } from "./brush.js";
...
export class Move {
  constructor(sprite) {
    ...
    this.startY = 0;
    this.toolOptions =
      "Click an drag to move on the canvas.  Try it with the select tool!";
    document.getElementById("TOOLOPTIONSCONTENT").innerHTML = this.toolOptions;
  }
...
export class Pencil {
  constructor(sprite) {
    ...
    this.toolOptions =
      "Left Click - Primary<br>Right Click - Secondary<br>Middle Click - Erase";
    document.getElementById("TOOLOPTIONSCONTENT").innerHTML = this.toolOptions;
  }
...
export class Brush {
  ...
  down(x, y, pri, sec, btn) {
    this.alpha = document.getElementById("COLOUROPACITY").value;
    let col = pri;
    if (btn == EVENT_MOUSEBUTTON_RIGHT) col = sec;
    ...
    this.brush.paint(
      ...
      this.sprite.getCurrentFrame().currentLayer,
      x,
      y,
      color,
      n255(this.alpha),
...
export class SprayCan {
  constructor(sprite) {
    ...
    this.toolOptions =
      "<div class='flux-slidercontainer'><input type='range' min='1' max='10' value='2' class='flux-slider' id='SPRAYCANRADIUS'></div><div class='flux-label sprayradiuslabel'>Spray Radius</div>";
    document.getElementById("TOOLOPTIONSCONTENT").innerHTML = this.toolOptions;
  }
  ...
  down(x, y, pri, sec, btn) {
    ...
    this.sprite.setPixelHex(
      x + (rand(this.radius * 2) - this.radius),
      y + (rand(this.radius * 2) - this.radius),
      col,
      this.alpha,
    );
  }
  ...
  drag(x1, y1, x2, y2, pri, sec, btn) {
    ...
    if (btn == EVENT_MOUSEBUTTON_RIGHT) col = sec;
    this.sprite.setPixelHex(
      x2 + (rand(this.radius * 2) - this.radius),
      y2 + (rand(this.radius * 2) - this.radius),
      col,
      this.alpha,
    );
  }
...
export class FloodFill {
  ...
  fillPixel(x, y) {
    ...
    if (
      pcol.r != pixelFlux.currentTool.fillingColour.r ||
    ...
    if (
      pcol.r != pixelFlux.currentTool.newColour.r ||
      pcol.g != pixelFlux.currentTool.newColour.g ||
      pcol.b != pixelFlux.currentTool.newColour.b
    ) {
      pixelFlux.currentTool.sprite.setPixelRGBA(
        x,
        y,
        pixelFlux.currentTool.newColour.r,
        pixelFlux.currentTool.newColour.g,
        pixelFlux.currentTool.newColour.b,
        pixelFlux.currentTool.alpha,
      );
...
export class DarkenLighten {
  constructor(sprite) {
    ...
    this.amount = 2;
    this.toolOptions =
      "<div class='flux-slidercontainer'><input type='range' min='1' max='10' value='2' class='flux-slider' id='DARKENLIGHTENAMOUNT'></div><div class='flux-label sprayradiuslabel'>Darken/Lighten Amount</div>";
    document.getElementById("TOOLOPTIONSCONTENT").innerHTML = this.toolOptions;
  }
