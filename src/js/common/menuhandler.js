  ...
  document.addEventListener("menuButtonClicked", (event) => {
    ...
    switch (srcElementId) {
      ...
      case "NEW100X100":
        ...
        break;
      case "NEW44X44":
        pixelFlux.createNewSprite(44, 44);
        pixelFlux.drawingScale = 12;
        pixelFlux.previewScale = 3;
        break;
      case "IMPORT":
        flux.showModalQuestionWindow(
          "Enter the URL of the image you would like to import: <br> <i>Note: Your current image will be replaced!</i>",
          "http://",
          "Import",
          "Cancel",
          (response) => {
      ...
      case "DOWNLOAD":
        pixelFlux.setSpriteName().then(() => {
          ...
          else download(url, pixelFlux.sprite.name + ".png");
        });
        break;
      case "DOWNLOADPIXELFILE":
        pixelFlux.setSpriteName().then(() => {
          var url =
            ...
            encodeURIComponent(JSON.stringify(pixelFlux.sprite));

          if (pixelFlux.sprite.name.length == 0)
            download(url, "pixelFlux-download.pixel");
          else download(url, pixelFlux.sprite.name + ".pixel");
        });
      ...
      case "DOWNLOADSPRITESHEET":
        pixelFlux.setSpriteName().then(() => {
          ...
          var dataUrl =
            pixelFlux.sprite.spriteSheetCanvas.toDataURL("image/png");
