...
export class PixelEditor {
  ...
  setupUI() {
    ...
    this.flux.loadMenu(menuUrl, (id) => {
      const postEvent = new CustomEvent("menuButtonClicked", {
        detail: { srcElementId: id },
      });
  ...
  addEventListeners() {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (srcElement.matches(".flux-toolbarbutton")) {
        const postEvent = new CustomEvent("toolButtonClicked", {
          detail: { srcElementId: srcElement.id, srcElement: srcElement },
        });
        document.dispatchEvent(postEvent);
      }
    });
  ...
  createToolbarWindow() {
    ...
    this.flux.appendToolButton(
      "TOOLBAR",
      "SELECTION",
      appUrl + "resources/icons/selecttoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "MOVE",
      appUrl + "resources/icons/movetoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "PENCIL",
      appUrl + "resources/icons/penciltoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "ERASER",
      appUrl + "resources/icons/erasertoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "FLOODFILL",
      appUrl + "resources/icons/bucketfillicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "SPRAYCAN",
      appUrl + "resources/icons/spraycanicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "STRAIGHTLINE",
      appUrl + "resources/icons/straightlinetoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "DARKENLIGHTEN",
      appUrl + "resources/icons/lightendarkentoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "BLEND",
      appUrl + "resources/icons/blendtoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "SQUARE",
      appUrl + "resources/icons/squaretoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "FILLEDSQUARE",
      appUrl + "resources/icons/filledsquaretoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "ELLIPSE",
      appUrl + "resources/icons/ellipsetoolicon.png",
    );
    this.flux.appendToolButton(
      "TOOLBAR",
      "FILLEDELLIPSE",
      appUrl + "resources/icons/filledellipsetoolicon.png",
    );
    //this.flux.appendToolButton("TOOLBAR", "PLUGIN", appUrl + "resources/icons/plugin-icon.png")
  }
  ...
  objectClicked(id) {
    switch (id) {
      case "ADDPRIMARYCOLOUR":
        this.addColourToPallet(
          document.getElementById("PRIMARYCOLOURPICKER").value,
        );
        break;
      case "ADDSECONDARYCOLOUR":
        this.addColourToPallet(
          document.getElementById("SECONDARYCOLOURPICKER").value,
        );
        break;
    }
  }
  ...
  addColourToPallet(colour) {
    ...
    colourPreview.onclick = (event) => {
      ...
      if (event.shiftKey) {
        ...
        if (event.button == 0) {
          document.getElementById("PRIMARYCOLOURPICKER").value = rgb2hex(
    ...
    colourPreview.oncontextmenu = (event) => {
      ...
      document.getElementById("SECONDARYCOLOURPICKER").value = rgb2hex(
        event.srcElement.style.backgroundColor,
      );
    };
  ...
  saveColourPaletteAs() {
    ...
    this.flux.showModalQuestionWindow(
  ...
  selectSavedPalette() {
    this.flux.showModalSelectionWindow(
      "Please select your Palette to load...",
      this.getSavedPaletteList(),
      "Load",
      "cancel",
      (response) => {
        if (response) {
  ...
  getSavedPaletteList() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (o.type == "palette" && key != "defaultColourPallet") {
        // defaultColourPallet is used for colour persistence between sessions
  ...
  updateCanvasAndPreview(fullUpdate) {
    var now = Date.now();
    if (now - this.lastUpdate > 100) fullUpdate = true;
  ...
  renderAnimationPreview() {
    ...
    pixelFlux.sprite.drawAnimationToCanvasId(
    ...
    pctx.rect(
      imageOriginX - 1,
      imageOriginY - 1,
      pixelFlux.sprite.width * scale + 2,
      pixelFlux.sprite.height * scale + 2,
    );
    pctx.stroke();
  ...
  updateFrameNumberDisplay() {
    document.getElementById("CURRENTFRAMENUMBER").innerHTML =
      this.sprite.currentFrame + 1 + " / " + this.sprite.frames.length;
  }
  ...
  renderLayersWindow() {
    ...
    for (var i = layerCount - 1; i > -1; i--) {
      ...
      editlayername.onclick = (event) => {
        ...
        flux.showModalQuestionWindow(
          "Edit Layer Name:",
          frame.layers[id].name,
          "Save",
          "Cancel",
          (response) => {
            if (response) {
  ...
  inSelection(x, y) {
    if (this.currentSelection.enabled) {
      if (x < this.currentSelection.x1 || x > this.currentSelection.x2)
        return false;
      if (y < this.currentSelection.y1 || y > this.currentSelection.y2)
        return false;
    }
    return true;
  }
  ...
  setSpriteName() {
    return new Promise((resolve, reject) => {
      flux.showModalQuestionWindow(
        "Please enter a name for your Sprite...",
        this.sprite.name,
        "Save",
        "cancel",
        (response) => {
  ...
  downloadGif() {
    ...
    gif.on("finished", function (blob) {
      ...
      flux.addWindowContent(
        "GIFDISPLAY",
        "<br><br><center>Click to Download</center>",
      );
      gifPreview.onload = (e) => {
