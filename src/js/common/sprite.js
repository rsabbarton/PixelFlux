...
export class Sprite {
  ...
  drawAnimationToCanvasId(id, x, y, scale) {
    ...
    ctx.drawImage(
      this.frames[frameNumber].canvas,
      0,
      0,
      this.width,
      this.height,
    ...
    for (var f = 0; f < source.frames.length; f++) {
      ...
      for (var l = 0; l < source.frames[f].layers.length; l++) {
        ...
        for (var p = 0; p < source.frames[f].layers[l].pixels.length; p++) {
          this.frames[f].layers[l].pixels[p].red =
            source.frames[f].layers[l].pixels[p].red;
          this.frames[f].layers[l].pixels[p].green =
            source.frames[f].layers[l].pixels[p].green;
          this.frames[f].layers[l].pixels[p].blue =
            source.frames[f].layers[l].pixels[p].blue;
          this.frames[f].layers[l].pixels[p].alpha =
            source.frames[f].layers[l].pixels[p].alpha;
        }
  ...
  pushToUndoHistory() {
    ...
    if (this.history.length > pixelFlux.preferences.undoHistorySize) {
      this.history = this.history.slice(
        -pixelFlux.preferences.undoHistorySize,
        this.history.length,
      );
    }
  }
