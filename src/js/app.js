...
document.addEventListener("mousedown", (event) => {
  ...
  if (srcElement.matches(".drawingcanvas")) {
    ...
    var oX =
      ...
      (pixelFlux.sprite.width / 2) * pixelFlux.drawingScale;
    var oY =
      (srcElement.clientHeight - 20) / 2 -
      (pixelFlux.sprite.height / 2) * pixelFlux.drawingScale;
...
document.addEventListener("mousemove", (event) => {
  ...
  if (srcElement.matches(".drawingcanvas")) {
    ...
    if (pixelFlux.currentSelection.enabled) {
      ...
      var selectW =
        (pixelFlux.currentSelection.x2 - pixelFlux.currentSelection.x1 + 1) *
        pixelFlux.drawingScale;
      var selectH =
        (pixelFlux.currentSelection.y2 - pixelFlux.currentSelection.y1 + 1) *
        pixelFlux.drawingScale;
    ...
    if (
      dX >= 0 &&
      dY >= 0 &&
      dX < pixelFlux.sprite.width &&
      dY < pixelFlux.sprite.height
    ) {
      var ctx = srcElement.getContext("2d");
      ctx.beginPath();
